...
// --- Coupon Feature Added Below ---

// 네비게이션 바 HTML에 쿠폰 메뉴 추가 (app.js의 renderApp 함수 내)
function renderApp() {
  // 네비게이션 바와 콘텐츠 영역 렌더링
  app.innerHTML = `
    <nav class="navbar">
...
// 페이지 렌더링 분기에 쿠폰 페이지 추가 (app.js의 renderPage 함수 내)
function renderPage(page) {
  const contentEl = document.getElementById("content");
  ...
  switch(page) {
    ...
    case "bp":
      ...
      break;
    case "coupons":
      renderCouponsPage(contentEl); // 쿠폰 페이지 렌더링 추가
      break;
    case "search":
      renderSearchPage(contentEl);
      break;
